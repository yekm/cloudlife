...
static void move ( particle_t* p, struct state * st ) {
    ...
    if ( p->age > st->max_age ) {
        ...
        p->x = st->width*(sin(dir)) ;/*st->ring_radius  * sin(dir);*/
        p->y = st->line_height;/*st->ring_radius * cos(dir);*/
        p->xx = p->yy = p->vx = p->vy = 0;
        p->age = 0;

        if ( st->epoch == WHITE && st->color )
            st->colors[WHITE] = next_color(st, st->colors[WHITE] );
        if  ( st->epoch == BLACK && st->color && st->bicolor )
            st->colors[BLACK] = next_color(st, st->colors[BLACK] );
...
static void* binaryhorizon_init ( Display* display, Window window ) {
    XWindowAttributes xgwa;
    ...
    st->duration = (get_integer_resource(st->display, "duration", "Seconds"));
    st->color =  get_boolean_resource(st->display, "color", "Boolean");
    st->bicolor = get_boolean_resource(st->display, "bicolor", "Boolean");
    st->fade = get_boolean_resource(   st->display, "fade",    "Boolean");
    ...
    if (st->duration <= 0)
      ...
      st->duration *= 1 + frand (0.3);  /* dual screens not in lockstep */
...
static unsigned long binaryhorizon_draw ( Display* display, Window win, void *closure ) {
    ...
    if (st->duration &&
        now > st->start_time + st->duration) {
      st->start_time = now;
      st->epoch = WHITE;
      create_particles ( st );
      create_buffers ( st, display, st->screen, win, st->gc );
    }
    ...
    if ( random() % 10000 > 9975 ){   /* was 9950, I wanted it slower to change*/
        st->epoch = (st->epoch == WHITE) ? BLACK : WHITE;
        st->line_height = - abs((int) (frand1()*st->height / 2));
        if (st->epoch == WHITE){
                st->line_height = - st->line_height;
        }
    }
...
static void binaryhorizon_reshape ( Display* display, Window win, void *closure, unsigned int w, unsigned int h ) {
    struct state *st = (struct state *) closure;
...
static Bool binaryhorizon_event ( Display* display, Window win, void *closure, XEvent* event ) {
    struct state *st = (struct state *) closure;
...
/* delete everything */
static void binaryhorizon_free ( Display* display, Window win, void *closure ) {
    struct state *st = (struct state *) closure;
    XWindowAttributes tmp;
    XGetWindowAttributes(display, win, &tmp);
...
static const char* binaryhorizon_defaults [] = {
    ".background:	black",
    ".foreground:	white",
    "*growth_delay:     10000",
...
static XrmOptionDescRec binaryhorizon_options [] = {
    ...
    { "-monocolor",        ".bicolor",          XrmoptionNoArg,  "False" },
...
XSCREENSAVER_MODULE("BinaryHorizon", binaryhorizon)
